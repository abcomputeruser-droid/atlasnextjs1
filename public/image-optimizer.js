...
function isOptimizableLocalImage(src) {
  return Boolean(
    src &&
      ...
      (src.startsWith("assets/images/") || src.startsWith("/assets/images/")) &&
      !src.endsWith(".svg")
  );
}
...
let optimizerBooted = false;

function bootImageOptimizer() {
  if (optimizerBooted) return;
  optimizerBooted = true;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", bootImageOptimizer, { once: true });
} else {
  bootImageOptimizer();
}
