const DATA_URL = "/motherboard.json";

const ROUTE_MAP = {
  "about-atlas.html": "/about",
  "all-products.html": "/all-products",
  "company.html": "/company",
  "index.html": "/",
  "monitor.html": "/monitors",
  "motherboard.html": "/motherboards",
...
function toRoutePath(value) {
  if (!value || value === "#" || /^(https?:)?\/\//i.test(value) || value.startsWith("#")) return value;
  if (ROUTE_MAP[value]) return ROUTE_MAP[value];
...
function renderNav(items) {
  const nav = el("motherboardNav");
  if (!nav) return;
  nav.innerHTML = "";
  items.forEach((item) => {
    const a = document.createElement("a");
    a.href = toRoutePath(item.href);
...
function renderHero(hero) {
  ...
  image.src = toAssetPath(hero.image);
...
function renderProducts(title, items, tag) {
  ...
  items.forEach((item) => {
    ...
    const image = toAssetPath(item.image);
    const url = toRoutePath(item.url);
    card.innerHTML = `
      ...
      <div class="product-card-body">
        ...
        ${url ? `<a class="cta stretched-link" href="${url}">View Details</a>` : ""}
      </div>
    `;
...
function renderFooter(footer) {
  ...
  footerContent.innerHTML = `
    ...
    <div>
      <h4>Resources</h4>
      ${footer.resources.map((link) => `<a href="${toRoutePath(link.href)}">${link.label}</a>`).join("")}
    </div>
    <div>
      <h4>Follow us</h4>
    ...
    <div>
      <h4>Legal</h4>
      ${footer.legal.map((link) => `<a href="${toRoutePath(link.href)}">${link.label}</a>`).join("")}
    </div>
  `;
...
async function init() {
  try {
    ...
    console.error("Atlas motherboards: failed to load content.", err);
  }
}
